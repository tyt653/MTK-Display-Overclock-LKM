# PMB110 185Hz 全链路运行时模块

英文说明：[README.md](README.md)

本项目是 PMB110 屏幕运行时超频模块的源码参考实现。模块在系统运行时
向 Android 显示栈增加 170Hz 和 185Hz 档位，覆盖官方提供的两种分辨率。
启用后，所有档位使用统一的 1496Mbps MIPI DSI 链路；官方档位通过补偿
...
本项目只针对已验证的 PMB110 基线。高刷新率可能超出面板 DDIC、DSI PHY、触控
同步、功耗或温度设计范围；即使模式枚举成功，也不代表长期稳定或硬件安全。
任何其他设备都应建立自己的源码、ABI、校验和恢复流程，不要直接复用本项目
生成的 `.ko`。
